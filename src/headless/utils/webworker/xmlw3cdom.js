    ...
    if (iEvt == XMLP._ELM_B) {                      // Begin-Element Event
      ...
      if (!this.namespaceAware) {
        ...
        for(var i = 0; i < p.getAttributeCount(); i++) {
          ...
          //TOFIND REMOVED ELSE HERE UP TO AFTER _isIdDeclaration check
            iAttr = iNode.getAttributeNode(strName);        // if Attribute exists, use it
...
DOMNode.prototype.getElementsByTagNameNS = function DOMNode_getElementsByTagNameNS(namespaceURI, localName) {
  ...
  let returnee = this._getElementsByTagNameNSRecursive(namespaceURI, localName, new DOMNodeList(this.ownerDocument));
  return returnee;
};
